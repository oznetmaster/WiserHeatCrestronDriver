...
public sealed partial class GatewayUiTests
	...
	private sealed partial class ScheduleSaveSession (GatewayUiTests fixture, HubSettings hub, HttpClient http,
		ControlRoomBinding control, RoomBinding binding, DeviceInfo original, string check) : IScheduleConflictSession, IScheduleLayoutSession, IScheduleRenameSession, IScheduleMembershipSession
		{
		...
		public async Task RecordAsync (string phase, object value)
			...
			file.Flush (true);
			if (phase is "create-intent" or "existing-schedule-intent" or "rename-intent" or "membership-create-intent")
