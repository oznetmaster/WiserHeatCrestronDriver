...
The Save Day/Save All session now retains a separate `save-day-response.json` or `save-all-response.json` before screenshot capture, page closing and reopening. It records the tap-helper request/return, matching idle driver command activity, an independent complete hub schedule, and monotonic elapsed times. The complete saved schedule must match the declared change; label feedback alone is insufficient.
