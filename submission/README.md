...
The help source now targets submission candidate 1.3.9, which is not yet an approved portal submission or public release. Its illustrated PDF is embedded in the opt-in candidate package. The published 1.3.8 package is unchanged. Ordinary builds retain their existing package name and do not require document tools. The candidate preserves the driver GUID and approved GitHub support website with an empty Email field.
...
The 1.3.9 candidate completed this opt-in Release build with official ManifestUtil 29. Its matching package/DLL/metadata/help names, embedded PDF bytes and dependency notices were verified. Every PDF page was visually inspected, preserving the official template and approved figure bytes. Candidate-bound hardware acceptance, endurance and final signed submission remain incomplete; successful packaging does not satisfy those requirements.
...
After a dependency update, review its package license and upstream notices before updating the inventory; a new DLL hash alone is not a license review. The current inventory matched all 18 dependency DLLs in the development build. The 1.3.9 candidate build verified inclusion and exact notice bytes after ManifestUtil. Retain this verification for the exact package later tested; do not transfer it to a rebuilt package without checking again. See [the reusable notice-build documentation](https://github.com/oznetmaster/CrestronHomeDevTools/blob/main/docs/submission/DependencyNotices.md).
