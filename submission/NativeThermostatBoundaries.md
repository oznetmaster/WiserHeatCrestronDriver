...
Candidate 1.3.14 advertises a 1 F input step, while Celsius uses 0.5 C. Fahrenheit requests are interpreted on the displayed whole-degree grid and mapped to the nearest supported hub half-degree. The test requires the corresponding exact physical target and matching rendered feedback after each input; it stops once the real requested endpoint is observed. It does not assume every Fahrenheit press changes the hub by exactly 0.5 C or permit values outside the supported range. Hardware validation of this candidate remains pending.
