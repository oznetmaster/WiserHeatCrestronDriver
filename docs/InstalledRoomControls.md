...
The first supported probe observes the existing `enableSchedule` and `disableSchedule` commands. It requires a selected room initially following an assigned schedule in Auto mode, without an override or boost. An explicit existing manual setpoint is required and must be between 5°C and the current scheduled target. The test temporarily selects Manual mode, then returns to Auto. It verifies the current schedule is controlling the room again and that the schedule assignment and guarded room settings are unchanged. It does not test temperature setters or edit schedules.
